...
class TDNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([x[:,:,0].reshape(-1,40,1).repeat(1,1,13), x, x[:,:,-1].reshape(-1,40,1).repeat(1,1,13)], 2)
...
class TDNN_LSTM(nn.Module):
    def __init__(self, feat_dim=40, hidden_size=1024):
        super(TDNN_LSTM, self).__init__()
        self.conv_1 = nn.Conv1d(feat_dim, hidden_size, 5)
        self.bn_1 = nn.BatchNorm1d(hidden_size, affine=False)
        self.conv_2 = nn.Conv1d(hidden_size, hidden_size, 1)
        self.bn_2 = nn.BatchNorm1d(hidden_size, affine=False)
        self.conv_3 = nn.Conv1d(hidden_size, hidden_size, 5, dilation=2)
        self.bn_3 = nn.BatchNorm1d(hidden_size, affine=False)
        self.conv_4 = nn.Conv1d(hidden_size, hidden_size, 1)
        self.bn_4 = nn.BatchNorm1d(hidden_size, affine=False)
        self.conv_5 = nn.Conv1d(hidden_size, hidden_size, 3, dilation=3)
        self.bn_5 = nn.BatchNorm1d(hidden_size, affine=False)
        self.conv_6 = nn.Conv1d(hidden_size, hidden_size, 1)
        self.bn_6 = nn.BatchNorm1d(hidden_size, affine=False)
        self.conv_7 = nn.Conv1d(hidden_size, hidden_size, 3, dilation=4)
        self.bn_7 = nn.BatchNorm1d(hidden_size, affine=False)
        """
        self.conv_8 = nn.Conv1d(hidden_size, hidden_size, 1)
        self.bn_8 = nn.BatchNorm1d(hidden_size, affine=False)
        self.conv_9 = nn.Conv1d(hidden_size, hidden_size, 1)
        self.bn_9 = nn.BatchNorm1d(hidden_size, affine=False)
        """
        # lstm
        self.p = 0.0
        self.lstm = nn.LSTM(hidden_size, 128, num_layers=1, batch_first=True,
                bidirectional=True)

        # self.dense_10 = nn.Linear(6048, 512)
        self.dense_10 = nn.Linear(2*128, 128)
        self.dense_11 = nn.Linear(128, 128)
        self.dense_12 = nn.Linear(128, 1)  # if softmax (64, 2)
        # self.last_layer = nn.Softmax(dim=-1)
        self.last_layer = nn.Sigmoid()
        self.drop_layer = nn.Dropout(p=self.p)

        # cmvn set
        self.LC = 150
        self.RC = 149

    def cmvn(self, x, norm_vars=False):
        # do cmvn
        B, dim, N = x.shape
        LC, RC = self.LC, self.RC
        win_len = min(N,  LC+RC+1)
        win_start = torch.arange(-LC, N-LC)
        win_start[win_start < 0] = 0
        win_start[win_start > N-win_len] = N-win_len
        f = torch.cat([torch.zeros(B, dim, 1).to(x.device), x.cumsum(-1)], -1)
        x = x - (f[:, :, win_start+win_len]-f[:, :, win_start])/win_len
        if norm_vars:
            f = torch.cat([torch.zeros(B, dim, 1).to(x.device), (x**2).cumsum(-1)], -1)
            x /= torch.sqrt((f[:, :, win_start+win_len]-f[:, :, win_start])/win_len)
        return x

    def forward(self, x):
        # import ipdb; ipdb.set_trace()
        x = self.cmvn(x, norm_vars=False).detach()
        ...
        out = self.conv_3(out)
        out = self.drop_layer(F.relu(out))
        out = self.bn_3(out)
        ...
        out = self.drop_layer(F.relu(out))
        out = self.dense_11(out)
        out = self.drop_layer(F.relu(out))
        out = self.dense_12(out)

        act = self.last_layer(out)
...
class LSTM_FD(nn.Module):
    def __init__(self, feat_dim=40, dropout=0.0):
        super(LSTM_FD, self).__init__()
        # lstm
        self.p = dropout
        self.lstm = nn.LSTM(feat_dim, 128, num_layers=2, batch_first=True,
